...
import { SiteConfig, GhostContentAPI as TypeGhostContentAPI } from "@/types";
...
export default class App extends Vue {
  ...
  async mounted(): Promise<void> {
    ...
    const api: typeof TypeGhostContentAPI = new GhostContentAPI({
      url: siteConfig.API_URL,
      key: siteConfig.CONTENT_API_KEY,
      version: "v3",
    });

    await this.$store.commit("SET_API", api);

    this.$store.dispatch("browseSettings");

    this.$store.dispatch("getAuthors");

    this.$store.dispatch("browseTags", "all");

    this.$store.dispatch("getFeaturedPosts", 10);
  }
  ...
  async handleRouteChange(route: { name: string }): Promise<void> {
    if (route.name === "post") {
    ...
    if (route.name === "posts") {
      setTimeout(async () => {
        this.$store.dispatch("getPostsTillPage", this.$route.params.pageNumber);
      }, 200);
    }
